// AI Case evaluation & settlement guidance
// Analyze facts, estimate likely award, suggest settlement range
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISmallClaimsCourtGuide'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISmallClaimsCourtGuide. Focus area: Case evaluation & settlement guidance. ${`Analyze facts, estimate likely award, suggest settlement range`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Case evaluation & settlement guidance.\n${`Analyze facts, estimate likely award, suggest settlement range`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'case-evaluation', context, parsed);
    res.json({ feature: 'case-evaluation', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[case-evaluation]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['case-evaluation']);
